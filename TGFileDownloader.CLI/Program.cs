...
internal partial class Program
{
    ...
    static async Task Download(MultiThreadDownloader downloader, PhotoBase photo, long msgId, long msgAuthor, long msgChannel)
    {
        switch (photo)
        {
            case Photo p:
                {
                    ...
                    else
                    {
                        ...
                        Storage_FileType type = await downloader.DownloadFileAsync(p, fileStream, ReportProgress, msgId, msgAuthor, msgChannel);
                        Console.WriteLine();
    ...
    static async Task Download(MultiThreadDownloader downloader, DocumentBase document, long msgId, long msgAuthor, long msgChannel)
    {
        switch (document)
        {
            case Document d:
    ...
    static async Task Main()
    {
        ...
        MultiThreadDownloader downloader = new(client, 10, 1024 * 1024, (c) =>
        {
        ...
        for (; ; )
        {
            ...
            foreach (var msgBase in messages.Messages)
            {
                ...
                if (msgBase is Message msg && msg.media is not null)
                {
                    ...
                    await ProcessMedia(downloader, msg.media, msg.ID, from.ID, chatId);
                }
            }
    ...
    static async Task ProcessMedia(MultiThreadDownloader downloader, MessageMedia msgMedia, long msgId, long msgAuthor, long msgChannel)
    {
        try
        {
            switch (msgMedia)
            {
                case MessageMediaPhoto photo:
                    ...
                    await Download(downloader, photo.photo, msgId, msgAuthor, msgChannel);
                    break;
                case MessageMediaDocument document:
                    Console.WriteLine("[Document]");
                    Console.WriteLine(document.document.ID);
                    await Download(downloader, document.document, msgId, msgAuthor, msgChannel);
                    break;
                case MessageMediaPaidMedia paidmedia:
                    ...
                    {
                        await ProcessMedia(downloader, media.media, msgId, msgAuthor, msgChannel);
                    }
                    break;
